...
public class MapDrawer extends DefaultViewer {
    ...
    @Override
    public void redraw(MazePanel panel, int px, int py,
     ...
     */
    public void draw_map(MazePanel mazePan, int px, int py, int walk_step, int view_dx, int view_dy, boolean showMaze, boolean showSolution) {
        // dimensions of the maze
        final int mazew = mazeConfig.getWidth() ;
        final int mazeh = mazeConfig.getHeight() ;

        mazePan.setColor(Color.WHITE);
        ...
        for (int y = ymin; y <= ymax; y++)
            for (int x = xmin; x <= xmax; x++) {
                ...
                mazePan.setColor(seencells.hasWall(x,y, CardinalDirection.North) ? Color.WHITE : Color.GRAY);
                if ((seencells.hasWall(x,y, CardinalDirection.North) || showMaze) && theCondition)
                    mazePan.drawLine(nx1, ny1, nx2, ny1);
                ...
                mazePan.setColor(seencells.hasWall(x,y, CardinalDirection.West) ? Color.WHITE : Color.GRAY);
                if ((seencells.hasWall(x,y, CardinalDirection.West) || showMaze) && theCondition)
                    mazePan.drawLine(nx1, ny1, nx1, ny2);
            }

        if (showSolution) {
            draw_solution(mazePan, offx, offy, px, py) ;
        }
    }
    /**
     ...
     */
    public void draw_currentlocation(MazePanel mazePan, int view_dx, int view_dy) {
        mazePan.setColor(Color.RED);
        ...
        mazePan.fillOval(ctrx-cirsiz/2, ctry-cirsiz/2, cirsiz, cirsiz);
        ...
        // now the drawing
        mazePan.drawLine(ctrx, ctry, aptx, apty);
        mazePan.drawLine(aptx, apty, aptx2 + ptofx, apty2 + ptofy);
        mazePan.drawLine(aptx, apty, aptx2 - ptofx, apty2 - ptofy);
    }
     ...
     * @param offy
     */
    public void draw_solution(MazePanel mazePan, int offx, int offy, int px, int py) {
        ...
        while (distance > 1) {
            ...
            mazePan.drawLine(nx1, ny1, nx2, ny2);
